...
import com.myCAFE.food.model.NewCart;
import com.myCAFE.food.model.NewFood;
    ...
    public boolean addNewItem(MultipartFile file, String newFoodData) throws IOException {
        ...
        if (!file.isEmpty())
            if (saveFileToAssets(file)) {
                foodDao.save(new Food(newFood.getId(), newFood.getName(), newFood.getPrice(),
                        newFood.getQuantityAvailable(), "/assets/" + file.getOriginalFilename(), "", ""));
                // foodDao.save(new
                // Food(newFood.getId(),newFood.getName(),newFood.getPrice(),newFood.getQuantityAvailable(),"\\"+file.getOriginalFilename(),"",""));
            }
        return true;
    }
